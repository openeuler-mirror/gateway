...
- **密钥亲和（`key_affinity`）**
  ...
  - 节点负载失衡时自动迁移再平衡
- **语义路由（`auto_router`）**
  - 按请求内容（推理链、代码、工具调用等信号）自动分级
  - 简单请求不占用大模型算力，复杂请求不降级到小模型
  - 内置启发式分类，也可对接外部 ML 分类服务，失败自动回落本地
- **反馈式负载均衡**
  - 由实时负载与健康状况驱动选点，请求不堆积到热点节点
  - 容量优势节点自动分流，过载节点自动旁路
  - 迁移与再平衡过程可在面板观测
- **异常节点治理**
  - 连续失败自动熔断，主动探活自动上下线，恢复后自动回流，全程无需人工干预
  - 兜底路由 `"*"` 同样受保护

## 流量治理与可观测

- **流控**
  - 超限请求排队而非直接拒绝，VIP 密钥优先得到服务
  - 超大请求快速失败，不拖垮队列
  - 槽位随响应结束或客户端断连自动释放，不泄漏
- **三维限流与计费**
  - 密钥 / 团队双层套餐，counts / tokens / costs 三个维度独立限额
  - 支持终身累计配额与时段计划，跨午夜自动切档
- **全链路可观测**
  - 每个请求的 TTFT、耗时、排队时长、调度决策、KV 命中率全部落库可查
...
- **Agent 类型分析**
  ...
  - 面板按分钟粒度呈现近一小时趋势
- **离群样本分析**
  - 自动扫描延迟、排队、KV 命中率、错误率等指标中的离群样本，异常自己浮出水面，不靠人工翻日志
  - 支持按密钥 / 模型 / 节点 / 团队四个维度下钻，直接定位异常源头
  - 结果按严重程度排序，最突出的问题排在最前
...
- **单实例支撑超高 QPS** — 常规规模无需集群、无需横向扩展，一个进程即可承载大规模流量，部署与运维成本最小化
- **可观测全开不降级** — 全量请求日志写库、完整 prompt 落盘（自动压缩轮转）、OTLP 上报全部开启的情况下，网关自身开销依然可控：非流式 p50 约 10ms、p999 ≤ 36.5ms
- **高负载下延迟平稳** — 2000 RPS 压测下非流式延迟无明显劣化，瓶颈始终在上游推理而非网关
- **数据详见下方实测**，可一键复现
